...
import { Lock, Mail, User, Building2, Loader2, ArrowRight, AlertCircle, Eye, EyeOff, Globe, Shield } from 'lucide-react';
      ...
      <div className="relative mx-auto flex min-h-screen w-full max-w-7xl items-center justify-center">
        ...
        <aside className="hidden lg:flex w-[50%] flex-col justify-center px-12 py-16 bg-gradient-to-br from-white/80 via-blue-50/50 to-white/70 backdrop-blur-xl border-r border-slate-200 min-h-screen">
          <div className="space-y-10">
            ...
            <div className="space-y-6">
              <div className="space-y-2">
                ...
                <h1 data-translate="registerSidebarTitle" className="text-5xl font-black leading-tight text-slate-900">Pendaftaran Ahli KSNSSB</h1>
              </div>
              <p data-translate="registerSidebarDesc" className="text-lg text-slate-700 leading-relaxed max-w-md">Daftar akaun anda dengan selamat untuk menyertai proses pengundian Kesatuan Sekerja Namicoh Suria Sdn Bhd yang telus dan berintegriti.</p>
            </div>

            <div className="glass-panel rounded-2xl p-6 border border-blue-200/30 bg-white/40 max-w-md">
              <p data-translate="securityTitle" className="font-bold text-slate-900 mb-1">Keselamatan Bertaraf Profesional</p>
              <p data-translate="securityDesc" className="text-sm text-slate-700">Sistem pengundian digital yang dijamin selamat bagi melindungi hak suara setiap ahli kesatuan.</p>
            </div>
          </div>
          ...
          <div className="space-y-3 text-sm mt-10">
            ...
            <p data-translate="poweredBy" className="uppercase tracking-[0.25em] text-slate-500 font-bold text-xs">Dikuasakan oleh KSNSSB</p>
          </div>
        </aside>
        ...
        <main className="flex-1 flex items-center justify-center p-6 w-full">
          <div className="w-full max-w-md p-6 lg:p-10 rounded-3xl border border-slate-200/50 bg-white/60 backdrop-blur-md shadow-2xl">
            ...
            <form onSubmit={handleRegister} className="space-y-5">
              <div>
                <label data-translate="fullNameLabel" className="block text-sm font-bold mb-2 text-slate-800">Nama Penuh</label>
                <div className="relative">
                  <User className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400" size={18} />
                  <input type="text" data-translate-placeholder="fullNamePlaceholder" placeholder="Masukkan nama penuh anda" value={fullName} onChange={(e) => setFullName(e.target.value)} required className="w-full rounded-2xl border border-slate-300 bg-slate-50/80 px-12 py-4" />
                </div>
              </div>
              <div>
                <label data-translate="companyLabel" className="block text-sm font-bold mb-2 text-slate-800">Syarikat / Organisasi</label>
                <div className="relative">
                  <Building2 className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400" size={18} />
                  <input type="text" data-translate-placeholder="companyPlaceholder" placeholder="Masukkan nama syarikat" value={company} onChange={(e) => setCompany(e.target.value)} required className="w-full rounded-2xl border border-slate-300 bg-slate-50/80 px-12 py-4" />
                </div>
              </div>
              <div>
                ...
                <div className="relative">
                  ...
                  <input type="email" data-translate-placeholder="emailPlaceholder" placeholder="contoh@emel.com" value={email} onChange={(e) => setEmail(e.target.value)} required className="w-full rounded-2xl border border-slate-300 bg-slate-50/80 px-12 py-4" />
                </div>
              </div>
              <div>
                ...
                <div className="relative">
                  ...
                  <input type={showPassword ? 'text' : 'password'} data-translate-placeholder="passwordPlaceholder" placeholder="Sekurang-kurangnya 6 aksara" value={password} onChange={(e) => setPassword(e.target.value)} required className="w-full rounded-2xl border border-slate-300 bg-slate-50/80 px-12 py-4" />
              ...
              <button type="submit" disabled={loading} className="w-full rounded-2xl bg-gradient-to-br from-blue-700 to-indigo-800 px-6 py-4 text-white font-bold hover:shadow-lg transition-all flex items-center justify-center gap-2">
                {loading ? <Loader2 className="animate-spin" size={20} /> : <span>Daftar Sekarang</span>}
                {!loading && <ArrowRight size={18} />}
              </button>
            </form>

            <div className="mt-8 pt-8 border-t text-center text-sm">
              <span data-translate="alreadyHaveAccount">Sudah mempunyai akaun?</span>
              <button type="button" onClick={() => navigate('/login')} className="ml-2 font-bold text-blue-700 hover:underline" data-translate="loginHere">Log Masuk</button>
            </div>
          </div>
        </main>
      </div>
